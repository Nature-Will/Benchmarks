...
robowin_root = Path("/mnt/shared-storage-user/p1-shared/yujiale/code/benchmarks/RoboTwin")
...
def save_comparison_video(real_obs_list, imagined_video, action_history, save_path, fps=15):
    ...
    for i in range(n_frames):
        ...
        real_high = resize_h(cam_high, base_h)
        real_left = resize_h(cam_left, base_h)
        real_right = resize_h(cam_right, base_h)
        row_real = np.hstack([real_high, real_left, real_right])
        ...
        if imagined_video is not None and i < n_imagined:
            ...
            # Split T-shape decoded frame into 3 camera views:
            #   Top 1/3: left_wrist(H/3, W/2) | right_wrist(H/3, W/2)
            #   Bottom 2/3: cam_high(2H/3, W)
            fh, fw = img_frame.shape[:2]
            h_top = fh // 3
            pred_left = img_frame[:h_top, :fw // 2]
            pred_right = img_frame[:h_top, fw // 2:]
            pred_high = img_frame[h_top:]

            # Resize each predicted view keeping its aspect ratio,
            # then letterbox (black-pad) to match the real view's exact size
            ...
            row_imagined = np.hstack([
                letterbox(pred_high, base_h, real_high.shape[1]),
                letterbox(pred_left, base_h, real_left.shape[1]),
                letterbox(pred_right, base_h, real_right.shape[1])
            ])
        else:
            row_imagined = np.zeros((base_h, target_width, 3), dtype=np.uint8)
            cv2.putText(row_imagined, "No prediction", (target_width // 2 - 100, base_h // 2),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 100, 100), 2)

        row_imagined = np.ascontiguousarray(row_imagined)
        row_imagined = add_title_bar(row_imagined, "Predicted (High / Left / Right)")
...
def eval_policy(task_name,
    ...
    while succ_seed < test_num:
        ...
        while TASK_ENV.take_action_cnt<TASK_ENV.step_lim:
            ...
            ret = model.infer(dict(obs=first_obs, prompt=prompt, save_visualization=save_visualization, video_guidance_scale=video_guidance_scale, action_guidance_scale=action_guidance_scale, return_video=True)) #(TASK_ENV, model, observation)
            action = ret['action']
            if 'video' in ret:
        ...
        vis_dir = Path(args['save_root']) / 'visualization' / task_name / str(args['policy_name']) / str(args['task_config']) / str(args['ckpt_setting']) / str(current_time)
        ...
        save_comparison_video(
            ...
            fps=3  # ~29 frames / 11s real time ≈ 2.6, round to 3
        )
        if TASK_ENV.eval_video_path is not None:
        ...
        save_dir = Path(args['save_root']) / 'metrics' / task_name / str(args['policy_name']) / str(args['task_config']) / str(args['ckpt_setting']) / str(current_time)
        save_dir.mkdir(parents=True, exist_ok=True)
