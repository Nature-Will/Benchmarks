...
def generate_episode_descriptions(task_name: str, episodes: List[Dict[str, str]], max_descriptions: int = 1000000):
    ...
    Supports extra instruction types (abstract, minimal, wrong, etc.) from the task JSON.
    """
    ...
    # Identify additional instruction types beyond seen/unseen
    meta_keys = ('full_description', 'schema', 'preference', 'seen', 'unseen')
    extra_types = {k: v for k, v in task_data.items()
    ...
    for i, episode in enumerate(episodes):
        ...
        # Process extra instruction types (abstract, minimal, wrong, etc.)
        for itype, instructions in extra_types.items():
            filtered = filter_instructions(list(instructions), episode)
            if filtered:
                # Instructions have matching placeholders — do replacement
                descs = []
                for inst in filtered[:max_descriptions]:
                    descs.append(replace_placeholders(inst, episode))
                episode_result[itype] = descs
            else:
                # Plain-text instructions without placeholders — use as-is
                episode_result[itype] = list(instructions)

        all_generated_descriptions.append(episode_result)
