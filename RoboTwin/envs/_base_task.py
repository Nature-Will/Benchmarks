...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        # mplib 0.2.1 segfaults on Python 3.11; disable TOPP (falls back to
        # fixed 50-step interpolation).  CuroboPlanner handles motion planning.
    ...
    def take_action(self, action, action_type:Literal['qpos', 'ee']='qpos'):  # action_type: qpos or ee
        ...
        if action_type == 'qpos':
            ...
            try:
                ...
                # TOPP failed — use linear interpolation fallback
                topp_left_flag = True  # still set arm joints
                left_n_step = 50
                left_result = dict()
                left_interp = np.linspace(left_path[0], left_path[-1], left_n_step)
                left_result["position"] = left_interp
                left_result["velocity"] = np.zeros_like(left_interp)
            ...
            try:
                ...
                # TOPP failed — use linear interpolation fallback
                topp_right_flag = True  # still set arm joints
                right_n_step = 50
                right_result = dict()
                right_interp = np.linspace(right_path[0], right_path[-1], right_n_step)
                right_result["position"] = right_interp
                right_result["velocity"] = np.zeros_like(right_interp)
